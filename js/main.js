...
var ItemClick = function() {
	$('.ajax-load-page').on('click', function(e) {
		...
		if ( $('#-single-holder > div').length ) {
			$('#-single-holder > div').remove();
		} 

		TweenMax.to('.loader--wrap', 1, { top: '-50px', autoAlpha: 1, display: 'block', ease: Power4.easeOut });
	...
	$('body').on('click', '.js-close-', function() {
		...
		TweenMax.set('.-wrapper', { visibility: 'visible', height: 'auto' });
		TweenMax.to('.-single-inner', 1, { marginTop: '50px', opacity: 0,  display: 'none', onComplete() {
			TweenMax.to('.-wrapper', 1, { marginTop: '0px', autoAlpha: 1, position: 'relative' });
...
$(document).ajaxStop(function(){
	setTimeout(function(){
		TweenMax.to('.loader--wrap', 1, { top: '0px', autoAlpha: 0, ease: Power4.easeOut });	
	}, 400);
});

var loadSinglePage = function(id, href) {
	$.ajax({
		url: href,
		type: 'GET',
		success: function(html) {

			TweenMax.to('.-wrapper', 1, { marginTop: '50px', autoAlpha: 0, visibility: 'hidden', onComplete() {
				TweenMax.set('.-wrapper', { height: 0 });
			} })

			var pSingleHolder = $('#-single-holder');
	    	
			var getHTMLContent = $(html).find('.-single-wrap').html();

			pSingleHolder.append(
				'<div id="-single-'+id+
				'" class="-single-inner"><span class="unslate_co--close- js-close- d-flex align-items-center"><span class="close--label">Back to </span><span class="icon-close2 wrap-icon-close"></span></span>' + getHTMLContent + '</div>'
			);
			...
			setTimeout(function() {
				TweenMax.set('.-single-inner', { marginTop: '100px', autoAlpha: 0, display: 'none' });
				TweenMax.to('.-single-inner', .5, { marginTop: '0px', autoAlpha: 1, display: 'block', onComplete() {
